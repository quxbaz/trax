...
import {channels, blips, mixables, player, songPlayer} from 'trax'
...
class AudioPlayer {
  ...
  playSong(song) {
    ...
    const player = state.songPlayer
    song.data.forEach((line, y) => {
      line.forEach((channelId) => {
        ...
        channel.blips.forEach((blipId, x) => {
          this.playBlip(state, blipId, (combinedState) => ({
            ...combinedState,
            offset: combinedState.offset + (player.beatDuration * 16 * y) + (player.beatDuration * x),
          }))
        })
      })
    })
