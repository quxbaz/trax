...
const channel = (state={}, action) => {
  switch (action.type) {
    ...
    // case actionTypes.TOGGLE_BLIP_AT_POSITION:

    //   // if the position is undefined, create a blip, else toggle the blip

    //   const item = blip(state.blips[action.payload], action)

    //   return {
    //     ...state,
    //     blips: util.replaceAt(state.blips, action.payload, item)
    //   }
    default:
      return state
  }
