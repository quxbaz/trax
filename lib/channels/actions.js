...
const createBlipAt = (id, i, blipState={}) => (dispatch, getState) => {
  ...
  */
  ...
  const action = dispatch(blips.actions.createBlip({
    beat: i,
    color: channel.color,
    mixable,
    ...blipState,
...
const muteBlipAt = (id, i) => (dispatch, getState) => {
  ...
  */
  ...
  if (isNil(channel.blips[i]))
    return
  dispatch(blips.actions.muteBlip(channel.blips[i]))
}
...
const toggleBlipAt = (id, i) => (dispatch, getState) => {
  ...
  */
  ...
  const channel = selectors.getById(id)(state)
  if (!isNil(channel.blips[i])) {
    dispatch(blips.actions.toggleBlip(channel.blips[i]))
  } else {
...
export default {
  ...
  setBlipAt, createBlipAt, muteBlipAt, unmuteBlipAt,
  toggleBlipAt,
}
