...
import {sequencer, channels, blips, mixables} from 'trax'
  ...
  constructor({audioService, store, tickInterval}) {
    ...
    this.timer.on('tick', () => {
      ...
      channels.selectors.getEnabled(state).forEach((channel) => {
        const blipId = channel.blips[state.sequencer.currentBeat]
        if (!blipId)
          return
        const blip = blips.selectors.getById(blipId)(state)
        const mixable = mixables.selectors.getById(blip.mixable)
